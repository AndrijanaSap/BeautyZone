...
export class CategoryEditComponent  {
  ...
  ngOnInit(): void {
    ...
    if (categoryId) {
      this.categoryService.getCategoryWithServicesById(categoryId).subscribe(data => {
        ...
        if (data.jobPosition) this.updateForm.controls.jobPosition.setValue(data.jobPosition);
  ...
  onSubmit() {
    ...
    if (this.updateForm.controls.id.value)
      updateCategoryDto.id = this.updateForm.controls.id.value;

    if (this.updateForm.controls.jobPosition.value)
      updateCategoryDto.jobPosition = this.updateForm.controls.jobPosition.value;

    if (this.updateForm.controls.name.value)
      updateCategoryDto.name = this.updateForm.controls.name.value;

    if (this.services.length > 0) {
      updateCategoryDto.services = this.services.map(service => service.id);
    }
  
    this.categoryService.updateCategory(updateCategoryDto).subscribe(response => {
